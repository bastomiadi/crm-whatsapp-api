...
<div class="mb-6">
    ...
    <p class="text-gray-600">Update role and permissions</p>
</div>

<div class="bg-white rounded-lg shadow p-6">
    <form method="POST" action="{{ route('crm.roles.update', $role) }}">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug', $role->slug) }}" required
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500" {{ $role->is_default ? 'readonly' : '' }}>
                @error('slug')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_default" id="is_default" value="1" {{ old('is_default', $role->is_default) ? 'checked' : '' }}
                    class="w-4 h-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500" {{ $role->is_default ? 'disabled' : '' }}>
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" id="description" rows="2"
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">{{ old('description', $role->description) }}</textarea>
            </div>
        </div>
